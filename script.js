...
const tableWrapper = document.getElementById('table-wrapper'); // NOVO
...
async function entrarModoAdmin(user) {
    ...
    // CORREÇÃO: Cria a estrutura da tabela de admin
    tableWrapper.innerHTML = `
        <table>
            <thead>
                <tr>
                    <th>Projeto</th><th>Nº Chamado</th><th>Responsável</th><th>Situação Atual</th><th>Prazo</th><th>Prioridade</th><th>Priorizado Por</th><th>Ações</th>
                </tr>
            </thead>
...
function entrarModoPublico() {
    ...
    // CORREÇÃO: Cria a estrutura da tabela pública
    tableWrapper.innerHTML = `
        <table>
            <thead>
                <tr>
...
async function deletarProjeto(id, nome) {
    ...
    if (confirmacao) {
        ...
        else { console.log(`Projeto "${nome}" excluído com sucesso.`); carregarProjetos(true); }
    }
}
